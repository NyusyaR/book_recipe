...
class RecipesRepository:
    ...
    async def find_one(cls, id_recipe) -> Optional[RecipeID]:
        ...
        try:
            if id_recipe <= 0 or not isinstance(id_recipe, int):
                raise HTTPException(status_code=422, detail="Ошибка, id_recipe должен быть целым числом.")
    ...
    async def find_all(cls) -> Sequence[RecipesOrm]:
        ...
        async with new_session() as session:
            try:
                ...
                raise HTTPException(status_code=500, detail="Ошибка сервера при обработке запроса.")
    ...
    async def add_one(cls, name: RecipeAdd) -> RecipeID:
        ...
        async with new_session() as session:
            ...
            if existing_recipe:
                raise HTTPException(status_code=500, detail="Рецепт с таким названием уже существует")
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Ошибка при создании рецепта: дублирование записей: {err}")
            except SQLAlchemyError as err:
                await session.rollback()
                raise HTTPException(status_code=500, detail=f"Ошибка при сохранении рецепта: {err}")
